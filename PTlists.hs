...
allIntegersExcludingFirst a b = if a < b then tail[a..b] else tail[a,a-1..b]
...
integersUpDownUp a b c d = if a < b then (if b < c then (if c < d then [a..b] ++ tail[b..c] ++ tail[c..d] else [a..b] ++ tail[b..c] ++ tail[c,c-1..d]) else [a..b] ++ tail[b,b-1..c] ++ tail[c..d]) else [a,a-1..b] ++ tail[b..c] ++ tail[c..d]
...
isPerfect n = if sum [ x | x <- init[1..n], n `mod` x == 0] == n then True else False 
...
allPerfects n = [ x | x <- init[1..n], sum [ a | a <- init[1..x], x `mod` a == 0] == x]

...
charToPhoneDigit c
  | c `elem` "abc" || c `elem` "ABC" = 2
  | c `elem` "def" || c `elem` "DEF" = 3
  | c `elem` "ghi" || c `elem` "GHI" = 4
  | c `elem` "jkl" || c `elem` "JKL" = 5
  | c `elem` "mno" || c `elem` "MNO" = 6
  | c `elem` "pqrs" || c `elem` "PQRS" = 7
  | c `elem` "tuv" || c `elem` "TUV" = 8
  | c `elem` "wxyz" || c `elem` "WXYZ" = 9
...
numListToNum nums = (read :: String -> Int) (numString nums)
numString [] = ""
numString (x:num) = show x ++ (numString num)
